 ...
 * USAGE
 * 
 * • This module allows export from Mendix to Word (.docx) based on an outline given as JSON (nested structure).
 * • Find it as a Java action "Export to Word (Outline, nested)" in the Toolbox and place it in your microflow.
 ...
 * • This module demonstrates the usage, see the Domain Model and the demonstration microflow (ACT_ExportOutlineNestedDOCX).
 * 
 * WORD TEMPLATE
 * 
 ...
 * WORD CONTENTS
 * 
 * • Only 3 levels of heading nesting are allowed (which will result in Heading 1, Heading 2 Heading 3 styles)
 * • Only these types are allowed: title, heading, paragraph, points
...
public class Java_ExportOutlineNestedDOCX extends CustomJavaAction<IMendixObject>
{
	...
	public Java_ExportOutlineNestedDOCX(
		IContext context,
	...
	public IMendixObject executeAction() throws Exception
	{
		...
		ILogNode logger = Core.getLogger("Java_ExportOutlineDOCX");
        ...
        try {
            ...
            if (docParsed.getFirstPage() != null) {
                processContentList(docParsed.getFirstPage(), 1);
                insertPageBreak();
            }
            ...
            processContentList(docParsed.getPages(), 1);

            if (docParsed.getFinalPage() != null) {
                insertPageBreak();
                processContentList(docParsed.getFinalPage(), 1);
            }
	...
	{
		return "Java_ExportOutlineNestedDOCX";
	}
    ...
    private static class Content {
        ...
        public List<Content> getSubContents() { return subContents; }
    ...
    /**
     * Recursively processes a list of {@link Content} objects and generates the corresponding
     ...
     * and bulleted lists). If the content is a heading with sub-contents, the method calls itself recursively
     * to process the nested content at the next heading level.</p>
     *
     ...
     * @param level the current heading level, which is used to apply the appropriate heading style.
     *              The level is incremented recursively for nested headings.
     */
    private void processContentList(List<Content> contents, int level) {
        ...
        for (Content content : contents) {
            switch (content.getType()) {
                case "title":
                    ...
                    break;
                case "heading":
                    createParagraph(content.getContent(), "heading " + level);
                    if (content.getSubContents() != null) {
                        processContentList(content.getSubContents(), level + 1);
                    }
                    break;
                case "paragraph":
                ...
                default:
                    // Type unknown
                    Core.getLogger("Java_ExportOutlineDOCX").warn("Type " + content.getType() +
